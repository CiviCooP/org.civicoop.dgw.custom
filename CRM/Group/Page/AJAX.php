...
class CRM_Group_Page_AJAX {
  ...
  private static function filterGroups($groups) {
  	...
  	foreach($groups as $gid => $group) {
  		$valid = false;
  		if (!isset($group_checks[$gid])) {
  			$valid = true;
  		} else {
  			if ($userAdminDGW && $group_checks[$gid]['admin']) {
  				$valid = true;
  			} elseif ($userConsulent && $group_checks[$gid]['consulent']) {
  				$valid = true;
  			} elseif ($userDirBest && $group_checks[$gid]['dirbest']) {
  				$valid = true;
  			}
  		}
