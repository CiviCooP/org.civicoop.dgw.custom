  ...
  static function sendActivityCopy($clientId, $activityId, $contacts, $attachments = NULL, $caseId) {
    ...
    if ($caseId) {
      $activityInfo['fields'][] = array('label' => 'Case ID', 'type' => 'String', 'value' => $caseId);
        ...
        /*
         * DGW23 - toevoegen adres aan targetmail voor activiteiten / dossieractiviteiten
         */
        $targetIds = array( );
        /*
         * eerst alle target ids voor de activiteit ophalen
         */
        require_once 'CRM/Activity/BAO/ActivityTarget.php';
        $targetIds = CRM_Activity_BAO_ActivityTarget::retrieveTargetIdsByActivityId( $activityId );
        /*
         * vervolgens naam, primair adres en primair telefoon ophalen voor 
         * elk gevonden target
         */
        $targetNaamAdres = array( );
        $i = 0;
        foreach ( $targetIds as $targetID ) {
            $targetNaam = null;
            $targetAdres = null;
            $targetTel = null;
            $targetQry = "SELECT display_name FROM civicrm_contact WHERE id = $targetID";
            $daoNaam = CRM_Core_DAO::executeQuery( $targetQry );
            if ( $daoNaam->fetch() ) {
                $targetNaam = $daoNaam->display_name;
            }
            $adresQry = "SELECT * FROM civicrm_address WHERE contact_id = $targetID AND is_primary = 1";
            $daoAdres = CRM_Core_DAO::executeQuery( $adresQry );
            if ( $daoAdres->fetch() ) {
                $targetAdres = $daoAdres->street_address.", ".$daoAdres->postal_code.
                        " ".$daoAdres->city;
            }
            $telQry = "SELECT phone FROM civicrm_phone WHERE contact_id = $targetID AND is_primary = 1";
            $daoTel = CRM_Core_DAO::executeQuery( $telQry );
            if ( $daoTel->fetch() ) {
                $targetTel = $daoTel->phone;
            }
            $targetNaamAdres[$i] = trim( $targetNaam );
            if ( !empty( $targetAdres ) && !empty( $targetTel) ) {
                $targetNaamAdres[$i] .= " (".$targetAdres.", tel: ".$targetTel.")";
            } elseif ( !empty( $targetAdres ) ) {
                $targetNaamAdres[$i] .= " (".$targetAdres.")";
            }
            $i++;
        }
        /*
         * vervolgens een implode van het totaal in activityInfo zetten
         */
        $activityInfo['fields'][0]['value'] = implode( "; ", $targetNaamAdres );
        /*
         * end DGW23
         */
