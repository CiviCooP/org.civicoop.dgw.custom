{*
 ...
 | Date         :   16 March 2011                                     |
 | Marker       :   DGW11                                             |
 | Description  :   Action options for relation only if relation is   |
 |                  not hoofdhuurder, medehuurder or koopovereenkomst | 
 |                  partner                                           |
 | Date			:	22 March 2011									  |
 | Marker		:	CoreCorp11                                        |
 | Description  :	Remove text 'indicates ....'                      |
 +--------------------------------------------------------------------+
*}
       ...
       <a href="{crmURL p='civicrm/contact/view' q="action=view&reset=1&cid=`$rel.cid`"}">{$rel.name}</a>
          ...
          ($contactId eq $rel.contact_id_b and $rel.is_permission_b_a eq 1) }
            ...
            {/if}
                ...
                <td class="nowrap">
                {* DGW11 Edit relationship only if not hoofdhuurder, *}
                {* medehuurder or koopovereenkomst partner *}
                {if $rel.relation ne 'Hoofdhuurder' and $rel.relation ne 'Medehuurder' and $rel.relation ne 'Koopovereenkomst partner'}
                	{$rel.action|replace:'xx':$rel.id}</td>
                {/if}
                {* end DGW11 *}
                </td>
        ...
        {if $relationshipTabContext}
            <div id="permission-legend" class="crm-content-block">
            	{* CoreCorp11 remove text (not translated in .po) *}
